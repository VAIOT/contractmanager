...
/**
 * @title NDAManager
 * @dev Management of Non-Disclosure Agreements (NDAs) with dynamic fields
 */
...
contract NDAManager {
    ...
    // ============= MAPPINGS ============
    mapping(address => mapping(uint256 => mapping(string => string))) private userNDAs;
    ...
    event NDAFieldUpdated(uint256 contractId, string fieldName, string fieldValue);
  ...
  function addNDA(
        ...
        require(_fieldNames.length == _fieldValues.length, "Field names and values length mismatch");
    ...
    */
    function updateNDAField(
        ...
        require(_contractId > 0 && _contractId < nextContractId, "Invalid contract ID");
     ...
     */
    function deleteNDAField(address _owner, uint256 _contractId, string memory _fieldName) public onlyDeployer {
    require(bytes(userNDAs[_owner][_contractId][_fieldName]).length != 0, "Field does not exist");
     ...
     */
    function getUserContractIds(address _owner) public view returns (uint256[] memory) {
        return userContractIds[_owner];
    }

    /**
     * @notice Fetches all field names and their corresponding values for a given NDA.
     * @dev This function iterates through all the fields of a specified NDA and returns arrays of field names and values.
     * @param _owner Address of the NDA owner.
     ...
     */
     function getNDAFieldsAndValues(address _owner, uint256 _contractId) 
        public 
        view 
        returns (string[] memory fieldNames, string[] memory fieldValues) {
     ...
     */
    function fieldExists(address _owner, uint256 _contractId, string memory _fieldName) private view returns (bool) {
    string[] memory fields = ndaFieldNames[_owner][_contractId];
    for (uint256 i = 0; i < fields.length; i++) {
        if (keccak256(abi.encodePacked(fields[i])) == keccak256(abi.encodePacked(_fieldName))) {
            return true;
        }
    }
    return false;
    ...
    */
    function removeFieldName(address _owner, uint256 _contractId, string memory _fieldName) private {
    uint256 fieldIndex = 0;
    bool found = false;
    for (uint256 i = 0; i < ndaFieldNames[_owner][_contractId].length; i++) {
        if (keccak256(abi.encodePacked(ndaFieldNames[_owner][_contractId][i])) == keccak256(abi.encodePacked(_fieldName))) {
            fieldIndex = i;
            found = true;
            break;
        }
    ...
    if (found) {
        for (uint256 i = fieldIndex; i < ndaFieldNames[_owner][_contractId].length - 1; i++) {
            ndaFieldNames[_owner][_contractId][i] = ndaFieldNames[_owner][_contractId][i + 1];
        }
